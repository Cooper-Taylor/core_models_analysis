...
class AICurationUtils(ArgoUtils):
    ...
    # Argo internal model id for Claude Opus 4.8 (see GET /argoapi/v1/models).
    DEFAULT_ARGO_MODEL = "claudeopus48"

    def __init__(
        self,
        backend: Optional[str] = None,
        model: Optional[str] = None,
        argo_max_tokens: int = 20000,
        **kwargs: Any,
    ) -> None:
        ...
        Args:
            ...
            model: Argo model id used by the 'argo' backend. Defaults to
                    'claudeopus48' (Claude Opus 4.8), so curation calls go to
                    Claude Opus 4.8 through the Argo gateway via the same legacy
                    chat endpoint ArgoUtils already uses (the "old script").
            argo_max_tokens: max_tokens to send to Argo for Anthropic (Claude)
                    models, which require it. Kept below the 21k streaming
                    threshold so the non-streaming /chat/ endpoint is used.
            **kwargs: Additional keyword arguments passed to SharedEnvironment/ArgoUtils
        """
        # Route the Argo backend to Claude Opus 4.8 by default.
        if model is None:
            model = self.DEFAULT_ARGO_MODEL
        kwargs["model"] = model

        # Stored so _payload can attach max_tokens for Claude models on Argo.
        self.argo_max_tokens = argo_max_tokens
    ...
    def _payload(self, prompt: str, system: str) -> dict:
        """Build the Argo request body, adding max_tokens for Claude models.

        Anthropic (Claude) models on the Argo gateway require max_tokens, but the
        parent ArgoUtils GPT-style payload doesn't set it. Everything else (the
        legacy system+user messages shape, temperature) is unchanged from the old
        ArgoUtils.chat() path, so requests still go to the legacy chat endpoint in
        exactly the same fashion -- just targeting Claude Opus 4.8.
        """
        payload = super()._payload(prompt, system)
        if str(self.model).startswith("claude"):
            payload.setdefault("max_tokens", self.argo_max_tokens)
        return payload
