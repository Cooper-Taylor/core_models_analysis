...
def main(argv: Optional[list] = None) -> None:
    ...
    # On-disk FBA: prefer all-models source (filter to current panel) so
    # the result is correct for whatever panel selection currently lives
    # in results/selected_ids.txt — even if the kbcache was populated
    # against an earlier panel.
    all_models_ondisk_path = ANALYSIS_ROOT / "results" / "results.csv"
    if all_models_ondisk_path.exists():
        import csv as _csv
        ondisk_fba = []
        with open(all_models_ondisk_path) as fh:
            for row in _csv.DictReader(fh):
                if row["model_id"] in panel_set:
                    ondisk_fba.append({
                        "model_id": row["model_id"],
                        "growth_flux": float(row.get("growth_flux") or 0.0),
                        "grows": (row.get("grows") or "").lower() == "true",
                        "status": row.get("status") or "",
                    })
        print(f"  ondisk_fba sourced from results.csv ({len(ondisk_fba)} panel rows)")
    else:
        ondisk_fba = s.cache.load("fba_ondisk_v1")

    # Heuristic-baseline FBA: prefer all_models_baseline_fba.json
    # (post-fix, regenerated by build_all_models_impact.py); fall back to
    # the kbcache only when that file is missing.
    all_models_baseline_path = SITE_DATA / "all_models_baseline_fba.json"
    if all_models_baseline_path.exists():
        am_baseline = json.loads(all_models_baseline_path.read_text())
        heur_baseline_fba = [r for r in am_baseline if r["model_id"] in panel_set]
        print(f"  heur_baseline_fba sourced from all_models_baseline_fba.json "
              f"({len(heur_baseline_fba)} panel rows)")
    else:
    ...
    for v in manifest_in["variants"]:
        ...
        # Variant FBA: prefer the all-models output (which is post-fix
        # and exact for the current panel) over the panel-specific kbcache
        # entry (which was computed against whatever panel existed when
        # notebook 06 last ran — may be stale if the panel was re-selected).
        #
        # An *empty* all_models file is a meaningful "no models contain
        # any changed reaction for this variant" — variant FBA equals
        # baseline FBA for every model. We synthesize the panel rows from
        # the heuristic-baseline FBA in that case so the panel_fba grid
        # is non-empty and the n_flux/n_flip counts are correctly zero.
        all_models_path = SITE_DATA / f"all_models_variant_fba__{tag}.json"
        vfba = []
        vfba_source = "missing"
        am_rows = None
        if all_models_path.exists():
            try:
                am_rows = json.loads(all_models_path.read_text())
                vfba = [r for r in am_rows if r["model_id"] in panel_set]
                vfba_source = "all-models"
            except Exception:
                am_rows = None
        # Two no-op cases — both mean variant FBA == baseline FBA for
        # every panel model:
        #   (a) am_rows is the empty list — variant changes nothing in
        #       any of the 5,683 core models
        #   (b) am_rows is non-empty but the affected models all sit
        #       outside the current panel (panel intersection is empty)
        if am_rows is not None and not vfba:
            vfba = [
                {**r, "baseline_flux": r["growth_flux"],
                 "variant_flux":  r["growth_flux"],
                 "delta_flux":    0.0,
                 "baseline_grows": r["grows"]}
                for r in heur_baseline_fba
            ]
            vfba_source = "baseline (no panel intersection)"
        if not vfba:
            # Last-resort fallback to the kbcache panel-specific entry
            # (which is likely stale w.r.t. the current panel selection).
            try:
                kb = s.cache.load(f"fba_variant_{tag.replace('.', '_')}_v2")
                vfba = [r for r in kb if r["model_id"] in panel_set]
                vfba_source = "kbcache (possibly stale)"
            except KeyError:
                pass
        ...
        n_flux = 0
        for r in vfba:
            # Skip rows for models not in the current panel (defensive —
            # the all-models filter above already restricts, but the
            # kbcache fallback may contain stale-panel entries).
            if r["model_id"] not in set(panel_ids):
                continue
            # When sourced from all_models_variant_fba, baseline_flux /
            # baseline_grows are already attached; trust them over the
            # heuristic_baseline cache (which would be stale if the
            # baseline FBA was rebuilt with a code fix since notebook
            # 06's last run).
            if "baseline_flux" in r:
                b_grows = bool(r["baseline_grows"])
                b_flux = float(r["baseline_flux"])
            else:
                b = odi.get(r["model_id"], {})
                b_grows = bool(b.get("grows", False))
                b_flux = float(b.get("growth_flux", 0.0))
            v_flux = float(r.get("growth_flux", 0.0))
            d = {
                "model_id": r["model_id"],
                "baseline_grows": b_grows,
        ...
        print(f"  {tag:>12}  changed: {diff['n_changed']:>5}  "
              ...
              f"(panel FBA from: {vfba_source}, n_rows: {len(rows)})")
